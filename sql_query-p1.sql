...
-- Q.7 Write a SQL query to calculate the average sale for each month. Find out best selling month for each year
...
SELECT 
category, SUM(total_sale) AS net_sale, COUNT(total_sale)
FROM retail_sales
GROUP BY 1;
...
category,
gender,
COUNT(total_sale) AS number_of_orders
FROM retail_sales
GROUP BY 1, 2
ORDER BY 1
